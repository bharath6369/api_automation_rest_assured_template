package util;

public class Constant {
    public static String BASE_URL="https://test-test.test.ai";



    public static String END_POINT_TOKEN_GENERATE="/api/auth/auth/oauth/token";
    public static String END_POINT_USER_INFO="/api/auth/auth/user";
    public static String END_POINT_CREATE_OPERATOR="/api/operator/v1/operators/";
    public static String END_POINT_GET_OPERATOR_BY_ID="/api/operator/v1/operators/{operatorId}";


}
